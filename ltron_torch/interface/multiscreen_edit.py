...
class MultiScreenEditInterfaceConfig(Config):
    mode_loss_weight = 1.
    shape_loss_weight = 1.
    color_loss_weight = 1.
...
class MultiScreenEditInterface:
    def __init__(self, config, model, optimizer):
        self.config = config
        self.model = model
        self.optimizer = optimizer
        dummy_env = MultiScreenEditEnv(config)
    ...
    def tensor_to_actions(self, x, env, mode='sample'):
        ...
        for i in range(b):
            ...
            if self.config.factor_cursor_distribution:
                if mode == 23:
                    action['pick_cursor']['activate'] = True
                if mode == 24:
                    action['place_cursor']['activate'] = True
                last_mode = 24
            ...
            if action['pick_cursor']['activate']:
                import pdb
                pdb.set_trace()
            ...
            if action['place_cursor']['activate']:
                import pdb
                pdb.set_trace()
                #action['place_cursor']['position'] = ENV RAVEL THING(EVERYWHERE)
