...
from ltron.gym.envs.multiscreen_edit_env import MultiScreenEditEnv
...
from ltron_torch.interface.multiscreen_edit import (
    MultiScreenEditInterfaceConfig,
    MultiScreenEditInterface,
)

class MultiScreenEditStubnetInterfaceConfig(
    MultiScreenEditInterfaceConfig
):
...
class MultiScreenEditStubnetInterface(MultiScreenEditInterface):
    ...
    def observation_to_tensors(self, batch, pad):
        ...
        # process table images (for the stub)
        ...
        # process hand images (for the stub)
        ...
        # process table tiles
        initial_table_tiles, initial_table_tyx, initial_table_pad = (
            batch_deduplicate_from_masks(
                observation['initial_table_color_render'],
                observation['initial_table_tile_mask'],
                observation['step'],
                pad,
        ))
        initial_table_tiles = default_tile_transform(
            initial_table_tiles).to(device).contiguous()
        initial_table_t = torch.LongTensor(initial_table_tyx[...,0]).to(device)
        initial_table_yx = torch.LongTensor(
            initial_table_tyx[...,1:]).to(device)
        ...
        table_tiles = default_tile_transform(
            table_tiles).to(device).contiguous()
        table_t = torch.LongTensor(table_tyx[...,0]).to(device)
        table_yx = torch.LongTensor(table_tyx[...,1:]).to(device)
        ...
        initial_table_pad = torch.LongTensor(initial_table_pad).to(device)
        # this is so gross... it sets the initial table pad to 0 wherever we
        # do not have a first frame
        # (this is for evaluation where we don't want to include initial image)
        initial_table_pad = initial_table_pad * torch.any(table_t == 1, dim=0)
        
        table_pad = torch.LongTensor(table_pad).to(device)
        x['table_tiles'], x['table_pad'] = cat_padded_seqs(
            initial_table_tiles, table_tiles, initial_table_pad, table_pad)
        x['table_t'], _ = cat_padded_seqs(
            initial_table_t, table_t, initial_table_pad, table_pad)
        x['table_yx'], _ = cat_padded_seqs(
            initial_table_yx, table_yx, initial_table_pad, table_pad)
        ...
        if self.config.factor_cursor_distribution:
            ...
            pick = observation['pick_cursor']
            x['pick_cursor'] = torch.LongTensor(pick).to(device)
            
            place = observation['place_cursor']
            x['place_cursor'] = torch.LongTensor(place).to(device)
        else:
            x['pick_cursor'] = None
            x['place_cursor'] = None
        ...
        token_t = torch.LongTensor(observation['step']+1).to(device)
