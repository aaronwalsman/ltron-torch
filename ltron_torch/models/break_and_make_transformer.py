...
from ltron_torch.envs.utils import categorical_or_max, categorical_or_max_2d
...
class CrossAttentionDecoder(Module):
    def __init__(self, config):
        ...
        # build the positional encodings
        self.spatial_position_encoding = LearnedPositionalEncoding(
            config.decoder_channels, config.spatial_tiles+config.mode_tiles)
        self.temporal_position_encoding = LearnedPositionalEncoding(
        ...
        self.mode_decoder = Linear(
            config.decoder_channels, config.mode_channels)
    ...
    def forward(self, tq, pad_q, xk, tk, pad_k, use_memory=None):
        ...
        table_x = table_x.permute(0, 3, 1, 4, 2, 5, 6)
        table_x = table_x.reshape(s, b, th*uh, tw*uw, uc)
        ...
        hand_x = hand_x.permute(0, 3, 1, 4, 2, 5, 6)
        hand_x = hand_x.reshape(s, b, hh*uh, hw*uw, uc)
        
        # split off the mode tokens
        mode_start = hand_end
        mode_end = mode_start + self.config.mode_tiles
        mode_x = x[:,mode_start:mode_end].permute(0, 2, 1, 3)
        mode_x = self.mode_decoder(mode_x)
        
        return table_x, hand_x, mode_x
