...
class LtronVisualTransformer(nn.Module):
    ...
    def visualize(self,
        ...
        for i, image in enumerate(images):
            ...
            # release heatmap and release p map
            ...
            if 'expert' in observation:
                ...
                for j in range(num_expert_actions):
                    ...
                    expert_modes.add(expert_mode_name)
                ...
                if num_expert_actions:
                    step_data['too_hard'] = False
                    step_data['mode_correct'] = (mode_name in expert_modes)
                    step_data['expert_modes'] = list(expert_modes)
                    ...
                    if mode_name in expert_modes and mode_name in (
                        'remove', 'pick_and_place', 'rotate', 'translate'
                    ):
                        clickable_locations = []
                        for j in range(num_expert_actions):
                    ...
                    #        if expert_mode_name != mode_name:
                    #            continue
                    #        expert_release = expert_action['cursor']['release']
                    #        release_island = release_islands[
                    #            (i,*expert_release)]
                    #        y,x = torch.where(
                    #            release_islands[i] == release_island)
                    #        releasable_locations.extend(zip(y,x))
